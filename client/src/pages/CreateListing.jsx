...
const CreateListing = () => {
    ...
    const handleRemove = (index)=>{
        setFormData({
            ...formData,
            imageUrls:formData.imageUrls.filter((image,id)=>id !== index)
        })
    }
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
        ...
        <div className='flex flex-col gap-4 flex-1'>
            ...
            className='border p-3 rounded-lg' maxLength='60'
            ...
            <p className='text-red-700 text-sm '>{imageUploadError && imageUploadError}</p>
            {
                formData.imageUrls.length > 0 && formData.imageUrls.map((url,index)=>{
                    ...
                    <div key={index} className='flex justify-between p-3'>
                        ...
                        <button onClick={()=>handleRemove(index)} type='button' className='text-red-700 uppercase'>delete</button>
                    </div>

                    )
                })
            }
        <button disabled={loading || uploading} className='p-3 bg-slate-700 rounded-lg 
